function checkToken() {
    ...
    if (token && role != "admin") {
        var endpoint;
        if (role == 'client') endpoint = 'http://localhost:4567/api/me'
        else if (role == 'shop') endpoint = 'http://localhost:4567/api/shop/parts'
        fetch(endpoint, {
            headers: { token: token }
        })
            ...
            .then((result) => {
                if (result.status == 401 || result.status == 403) {
                    ...
                    var fullAvatarUrl = "http://localhost:4567/" + avatarUrl;
