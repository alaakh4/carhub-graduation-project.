define([
    ...
    function FixYourCarViewModel() {
        ...
        self.viewShopProfile = function (shop) {
            ...
            window.location.href = 'workShopDetail/' + shop.id;
        };
        ...
        self.requestServiceFromShop = function (shop) {
            ...
            if (self.complaintText()) params.set('details', self.complaintText());
            if (self.predictedClass()) params.set('predictedClass', self.predictedClass());
            if (self.predictedLabel()) params.set('predictedLabel', self.predictedLabel());

            window.location.href = 'request-service?' + params.toString();
        };

        self.browseAllWorkshops = function () {
            if (self.predictedClass()) {
                window.location.href = 'workshops?services=' + encodeURIComponent(self.predictedClass());
                return;
            }
