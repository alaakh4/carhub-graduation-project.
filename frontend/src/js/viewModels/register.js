 ...
 * - User chooses role (client / workshop)
 * - User uploads image (NO manual URL input)
 * - After successful register, always redirect to #login
 */

define(['knockout', 'models/auth.model', 'models/city.model', 'ojs/ojarraydataprovider',
  'accUtils', 'ojs/ojselectsingle', 'ojs/ojselectcombobox', 'ojs/ojdialog'], function (ko, authModel, cityModel, ArrayDataProvider, accUtils) {
    function RegisterViewModel() {
       ...
       * Endpoint: POST http://localhost:4567/api/upload/image
      ...
      self._uploadImage = function (file, folder) {
        return new Promise(function (resolve, reject) {
          try {
            ...
            fetch('http://localhost:4567/api/upload/image', {
              method: 'POST',
              body: fd
            })
              .then(function (resp) {
        ...
        uploadPromise
          .then(function (finalPayload) {
            authModel.register(role, finalPayload, function (ok, res) {
              ...
              var msg;
              if (self.isShop()) {
                return self.openShopApproveDialog();
              } else {
                msg = 'Account created successfully. Please login.';
                self.successMsg(msg);
                setTimeout(() => router.go({ path: "login" }), 500);
              }
            });
          })
